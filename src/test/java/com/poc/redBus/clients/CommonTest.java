...
import com.poc.redBus.clients.PlatformTestClient;
import com.poc.constants.CommonConstants;
import com.poc.dataprovider.TestEnvironment;
import com.poc.helpers.DataHelper;

//import com.aptimus.altcloud.reporting.PerformanceData;
import com.poc.redBus.utils.EvaluateXPath;
import com.poc.redBus.utils.JsonToXMLMapper;
...
public abstract class CommonTest {
	...
	protected  Map<String,String> mapCookie;
	...
	com.poc.reporting.Reporter perfReporter;
	...
	public  void initialize (@Optional("") String theTestEnv, @Optional("")String theAppRequestContentType, @Optional("")String theApplicationResponseContentType,@Optional("OAUTH")String theAuthType, @Optional("") String theSeleniumServer,
		...
		testEnv = theTestEnv;
		if (testEnv != null){
			if (testEnv.equalsIgnoreCase(CommonConstants.KEY_NETWORK_PROTOCOL_REDBUS_APTIMUS)){
				host = prop.getProperty(CommonConstants.KEY_HOST_REDBUS_APTIMUS);
				if (prop.getProperty(CommonConstants.KEY_PORT_REDBUS_APTIMUS) != null && prop.getProperty(CommonConstants.KEY_PORT_REDBUS_APTIMUS).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_REDBUS_APTIMUS));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_REDBUS_APTIMUS);

			}
			else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_REDBUS_APTIMUS)){
				host = prop.getProperty(CommonConstants.KEY_HOST_REDBUS_APTIMUS);
				if (prop.getProperty(CommonConstants.KEY_PORT_REDBUS_APTIMUS) != null && prop.getProperty(CommonConstants.KEY_PORT_REDBUS_APTIMUS).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_REDBUS_APTIMUS));
				}
			...
			/*else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_QA_APTIMUS_SOLR)){
				host = prop.getProperty(CommonConstants.KEY_HOST_QA_APTIMUS_SOLR);
				if (prop.getProperty(CommonConstants.KEY_PORT_QA_APTIMUS_SOLR) != null && prop.getProperty(CommonConstants.KEY_PORT_QA_APTIMUS_SOLR).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_QA_APTIMUS_SOLR));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_QA_APTIMUS_SOLR);
			}*/
			/*else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_SJ_DEV)){
				host = prop.getProperty(CommonConstants.KEY_HOST_SJ_DEV);
				if (prop.getProperty(CommonConstants.KEY_PORT_SJ_DEV) != null && prop.getProperty(CommonConstants.KEY_PORT_SJ_DEV).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_SJ_DEV));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_METIS);
			}else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_METIS)){
				host = prop.getProperty(CommonConstants.KEY_HOST_METIS);
				if (prop.getProperty(CommonConstants.KEY_PORT_METIS) != null && prop.getProperty(CommonConstants.KEY_PORT_METIS).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_METIS));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_METIS);
			}else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_SJ_DEV)){
				host = prop.getProperty(CommonConstants.KEY_HOST_SJ_DEV);
				...
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_SJ_DEV);
			}else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_PROD)){
				host = prop.getProperty(CommonConstants.KEY_HOST_PROD);
				if (prop.getProperty(CommonConstants.KEY_PORT_SJ_DEV) != null && prop.getProperty(CommonConstants.KEY_PORT_SJ_DEV).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_PROD));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_PROD);
			}
			//acp-dev configuration
			else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_ACP)){
				host = prop.getProperty(CommonConstants.KEY_HOST_ACP_DEV);
				if (prop.getProperty(CommonConstants.KEY_PORT_ACP_DEV) != null && prop.getProperty(CommonConstants.KEY_PORT_ACP_DEV).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_ACP_DEV));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_ACP_DEV);
			}//acp-st configuration
			else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_ACP_ST)){
				host = prop.getProperty(CommonConstants.KEY_HOST_ACP_ST);
				if (prop.getProperty(CommonConstants.KEY_PORT_ACP_ST) != null && prop.getProperty(CommonConstants.KEY_PORT_ACP_ST).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_ACP_ST));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_ACP_ST);
			}else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_OTHER)){
				host = prop.getProperty(CommonConstants.KEY_HOST_OTHER);
				if (prop.getProperty(CommonConstants.KEY_PORT_OTHER) != null && prop.getProperty(CommonConstants.KEY_PORT_OTHER).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_OTHER));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_OTHER);
			}
			else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_PHX_QA1)){
				host = prop.getProperty(CommonConstants.KEY_HOST_PHX_QA1);
				if (prop.getProperty(CommonConstants.KEY_PORT_PHX_QA1) != null && prop.getProperty(CommonConstants.KEY_PORT_PHX_QA1).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_PHX_QA1));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_PHX_QA1);
			}
			else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_PORTAL)){
				host = prop.getProperty(CommonConstants.KEY_HOST_PORTAL);
				if (prop.getProperty(CommonConstants.KEY_PORT_PORTAL) != null && prop.getProperty(CommonConstants.KEY_PORT_PORTAL).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_PORTAL));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_PORTAL);
			}
			else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_STNEPTUNE)){
				host = prop.getProperty(CommonConstants.KEY_HOST_STNEPTUNE);
				if (prop.getProperty(CommonConstants.KEY_PORT_STNEPTUNE) != null && prop.getProperty(CommonConstants.KEY_PORT_STNEPTUNE).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_STNEPTUNE));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_STNEPTUNE);
			}
			else if (testEnv.equalsIgnoreCase(CommonConstants.TEST_ENVIRONMENT_STDEMO)){
				host = prop.getProperty(CommonConstants.KEY_HOST_STDEMO);
				if (prop.getProperty(CommonConstants.KEY_PORT_STDEMO) != null && prop.getProperty(CommonConstants.KEY_PORT_STDEMO).length() > 0) {
					port = Integer.parseInt(prop.getProperty(CommonConstants.KEY_PORT_STDEMO));
				}
				scheme = prop.getProperty(CommonConstants.KEY_NETWORK_PROTOCOL_STDEMO);
			}*/
		...
		if (thePerfLog != null && !thePerfLog.isEmpty()){
			perfReporter = new com.poc.reporting.Reporter(klassName.substring(this.getClass().getName().lastIndexOf(".")+1, klassName.length()));
